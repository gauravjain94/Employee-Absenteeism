#clear the contents of the global environment
rm(list = ls())

#Get the  Current working directory.
getwd()

#installing the required packages for the execution
x = c("ggplot2", "corrgram", "DMwR", "caret", "randomForest", "unbalanced", "C50", "dummies", "e1071", "Information",
      "MASS", "rpart", "gbm", "ROSE", 'sampling', 'DataCombine', 'inTrees','readxl')
lapply(x, require, character.only = TRUE)

# reading the data absenteeism at the work 
library("readxl")
...
# For train data from ggplot2 library 
library(ggplot2)
cl = ggplot(data ,aes(x = Absenteeism_time_in_hours)) + ggtitle("Absenteeism_time_in_hours")
print(cl + geom_bar(fill = 'red'))
...
#performing Normality Check on the data 
qqnorm(data$ID)
...
#Replace all the outliers with NA and impute the data
for(i in cnames)
{
...
#creating dummy variables for the data 
...
# Deleting the columns from the data for  which dummies are created
...
# Weight seems to be correlated, so Deleting the weight from the data
data = subset(data , select = -c(Weight))

#Splitting the data into train data and testdata
n = nrow(data)
...
#compute standard deviation of each principal component in the data 
